...
public class PantallaPrincipal extends JFrame implements Observer{
	...
	private class Controler implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			if (e.getSource().equals(PantallaPrincipal.this.botonJugar)) {
				Juego.getJuego().cargarJugadores(Integer.parseInt(PantallaPrincipal.this.numJugadores.getText()), Integer.parseInt(PantallaPrincipal.this.numNPCs.getText()), Integer.parseInt(PantallaPrincipal.this.numPokemons.getText()));
	...
	public void update(Observable arg0, Object arg1) {
		...
		PantallaJugador pj= new PantallaJugador(((Integer)datos[0]).intValue(), (String)datos[1], ((Integer)datos[2]).intValue());
		...
		for (int i = 0; i < Integer.parseInt(this.numPokemons.getText()); i++) {
			ListaJugadores.getListaJugadores().buscarJugador(nombre).getListaPokemons().getPosicionPokemon(i).addObserver((Observer)pj.getPanelPokemon().getComponent(i));
